...
async function fetchNationalParks() {
  try {
    ...
    const parksTable = $('caption:contains("List of U.S. national parks")').closest('table');
    ...
    parksTable.find('tr').each((index, element) => {
      ...
      // Name is in the first column, either in a th with scope="row" or td 
      const nameElement = $(element).find('th[scope="row"] a, td:first-child a').first();
      ...
      // Add to our parks array - use a fallback image if none is available
      if (name) {
        const defaultImage = "https://images.unsplash.com/photo-1506087712044-e3ab507abb43?w=600&auto=format&fit=crop";
        parks.push({
          name,
          description,
        ...
        console.log(`Found park: ${name} ${imageUrl ? '' : '(using default image)'}`);
      }
    });
