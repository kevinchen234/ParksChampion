...
async function fetchNationalParks() {
  try {
    ...
    // Find the main table with the parks data
    ...
    parksTable.find('tr').each((index, element) => {
      ...
      if (columns.length >= 7) {
        ...
        // Remove citation numbers [10] etc.
    ...
    const formattedParks = parks.map((park, index) => {
      ...
      return {
        ...
        rating: 1500, // Initial ELO rating for all parks
        rank: index + 1, // Initial rank based on order in Wikipedia
        previousRank: index + 1 // Initial previousRank is the same as current rank
      };
    });
